	...
	for(auto& field : fields){
		if(field.type == "childObject"){
			ret.append(
				...
				"\t{ // For scope\n"
				"\t\tif((watchChangeFlags == false) || (watchChangeFlags == true && (this->" + field.name + ".IsDirty() || this->" + field.name + ".ChildrenDirty())) ){\n"
				"\t\t\tbool childObjHasChanged = this->" + field.name + "." + field.function + "(db, &original->" + field.name + ", changeReason, ourKey, logChanges, watchChangeFlags);\n"
			...
			ret.append(
				...
				"\tif(watchChangeFlags == true){\n"
				"\t\tfor(auto c : *" + field.name + ") {\n"
				"\t\t\tif(c->IsNew()){\n"
				"\t\t\t\tc->" + field.function + "(db, nullptr, \"Insert\", ourKey, logChanges, watchChangeFlags);\n"
				"\t\t\t\tfoundChange = true;\n"
				"\t\t\t} else if(c->IsDeleted()){\n"
				"\t\t\t\tc->" + field.function + "(db, nullptr, \"Delete\", ourKey, logChanges, watchChangeFlags);\n"
				"\t\t\t\tfoundChange = true;\n"
				"\t\t\t} else if(c->IsModified() || c->ChildrenDirty()){\n"
				"\t\t\t\tfor(auto originalC : *original->" + field.name + "){\n"
				"\t\t\t\t\tif(Statics::IdMatch( c->Id, originalC->Id )){\n"
				"\t\t\t\t\t\tbool childHasChanged = c->" + field.function + "(db, originalC, changeReason, ourKey, logChanges, watchChangeFlags);\n"
				"\t\t\t\t\t\tif(childHasChanged){\n"
				"\t\t\t\t\t\t\tfoundChange = true; // Remember this\n"
				"\t\t\t\t\t\t}\n"
				"\t\t\t\t\t\tbreak; // Found the child to log against\n"
				"\t\t\t\t\t}\n"
				"\t\t\t\t}\n"
				"\t\t\t}\n"
				"\t\t}\n"
