...
class AnException {
	public:
		...
		AnException(int id, const char *file, int line, const char *msg, ...)
		{
			...
			/* ********************************************* */
			/* There is a potential with some exception code */
			/* generated by certain compilers that exceptions*/
			/* thrown at exactly the same time cause signal  */
			/* problems.  We are going to avoid this with the*/
			/* introduction of a microsecond sleep call here.*/
			/* The value of the sleep will be our current    */
			/* thread ID.  Which guarantees us that if another*/
			/* thread throws an exception at /exactly/ the   */
			/* same time as we do, then we will be offset a  */
			/* bit, comming out of this constructor method.  */
			/* Keep the sleep value small so that performance*/
			/* is not adversly affected.                     */
			/* ********************************************* */
			tid = (long int)CURRENT_THREAD_ID;			
			if(tid < 10000) 
				tid *= 10;
			else
				tid = (tid % 10000) * 10;
			
			SLib::Tools::sleep(tid);
